...
import type { RuntimeStateStore, DurableTurnRecord } from "../durability/runtime-state.js";
...
export class DurableTurn {
  ...
  readonly #tools: BufferedToolEvent[] = [];
  readonly #stagedEffects: Effect[] = [];
  readonly #effectResults = new Map<string, EffectResult>();
  readonly #options: DurableTurnOptions;
  #semanticExposed = false;
  #closed = false;
  #persistTail: Promise<void> = Promise.resolve();
  #persistError?: unknown;
  #record!: DurableTurnRecord;
  #baseSnapshot!: Awaited<ReturnType<MemoryWorkspace["snapshot"]>>;

  private constructor(options: DurableTurnOptions) {
    this.#options = options;
    this.id = options.turnId ?? `turn_${randomUUID()}`;
  }

  static async begin(options: DurableTurnOptions): Promise<DurableTurn> {
  ...
  stageEffect(effect: Effect): void {
    this.#assertOpen();
    if (this.#stagedEffects.some((existing) => existing.id === effect.id)) return;
  ...
  async executeEffect(effect: Effect): Promise<EffectResult> {
    ...
    if (mode === "commit") {
      ...
      const result: EffectResult = { ok: true, output: { staged: true, effectId: effect.id } };
      this.#effectResults.set(effect.id, result);
      return structuredClone(result);
    }

    if (!this.#options.executeEffect) return { ok: false, error: "No effect executor configured" };
    if (mode === "barrier") {
    ...
    const executionEffect = mode === "attempt-local"
      ? { ...effect, id: `${effect.id}::${this.id}` } as Effect
      : effect;
  ...
  async commit(): Promise<void> {
    ...
    for (const effect of this.#stagedEffects) {
      if (!this.#options.executeEffect) throw new Error(`Cannot commit staged effect ${effect.id}: no executor`);
      this.#semanticExposed = true;
      await this.#syncRecord();
    ...
    for (const tool of this.#tools) await this.#options.publishTool?.(tool);
    for (const output of this.#outputs) await this.#options.publishOutput?.(output);
  ...
  #scheduleSyncRecord(): void {
    if (!this.#record || this.#closed) return;
    this.#record = {
      ...this.#record,
      updatedAt: Date.now(),
      semanticExposed: this.#semanticExposed,
  ...
  #enqueueRecord(record: DurableTurnRecord): void {
    const store = this.#options.store;
    if (!store) return;
    const snapshot = structuredClone(record);
    this.#persistTail = this.#persistTail.then(async () => {
      if (this.#persistError !== undefined) return;
      try {
  ...
  async #persistTerminalRecord(): Promise<void> {
    this.#enqueueRecord(this.#record);
    await this.#flushPersistence();
  }

  #assertOpen(): void {
    if (this.#closed) throw new Error(`Turn ${this.id} is already closed`);
  }
}
...
export async function runDurableTransactionalTurn<T>(options: Omit<DurableTurnOptions, "attemptId"> & {
  attempts: readonly DurableTurnAttempt<T>[];
  onAttempt?: (event: { type: "start" | "rollback" | "commit" | "failed"; attemptId: string; turnId: string; error?: string }) => void;
}): Promise<T> {
  if (options.attempts.length === 0) throw new Error("At least one durable turn attempt is required");
...
export function defaultEffectReplayMode(effect: Effect): EffectReplayMode {
  switch (effect.kind) {
    case "workspace.read":
