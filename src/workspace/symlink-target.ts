...
export type SymlinkTargetResolution =
  ...
  | { ok: false; reason: "escapes"; linkPath: string; target: string }
  | { ok: false; reason: "cycle"; linkPath: string }
  | { ok: false; reason: "too-deep"; linkPath: string };
 ...
 */
export function resolveLinkTarget(linkPath: string, target: string): { resolved?: string; escapes: boolean } {
  if (isAbsoluteTarget(target)) return { escapes: true };
  ...
  for (const raw of target.replace(/\\/g, "/").split("/")) {
    if (!raw || raw === ".") continue;
    if (raw === "..") {
      if (depth === 0) return { escapes: true };
      depth--;
      parts.pop();
    } else {
      depth++;
