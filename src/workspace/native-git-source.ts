...
function parseLsTreeZ(buffer: Buffer): LsTreeEntry[] {
  ...
  for (const record of text.split("\0")) {
    ...
    out.push({ mode: left[0]!, type: left[1]! as LsTreeEntry["type"], objectId: left[2]!, name: record.slice(tab + 1) });
  }
  return out;
}
 ...
 */
export class NativeGitSource implements TreeSource {
  ...
  private constructor(options: NativeGitSourceOptions) {
  ...
  static async open(options: NativeGitSourceOptions): Promise<NativeGitSource> {
  ...
  async stat(path: string): Promise<SourceInfo | undefined> {
    ...
    if (!this.#relevant(p)) return undefined;
    if (!p) return { path: "", kind: "directory", size: 0, mtimeMs: 0 };
    await this.#ensureCommit();
    const { stdout } = await this.#run(["ls-tree", "-z", this.#commit!, "--", p], { encoding: "buffer" });
    const entries = parseLsTreeZ(stdout as Buffer);
  ...
  async listDir(path: string): Promise<readonly SourceInfo[]> {
    ...
    try {
      ...
      if (isMissingTreeish(error)) return [];
      throw error;
    }
  }
  ...
  async readFile(path: string): Promise<Uint8Array> {
    const p = normalizeRelative(path);
    if (!this.#relevant(p)) throw new Error(`Path outside sparse view: ${p}`);
    await this.#ensureCommit();
    const info = await this.stat(p);
    if (!info?.objectId) throw new Error(`Git path is not a file: ${p}`);
    if (info.size > this.#maxBlobBytes) throw new Error(`Git blob exceeds hydration limit (${info.size} > ${this.#maxBlobBytes}): ${p}`);
    const batch = this.#batch ??= new GitCatFileBatch({ gitDir: this.#gitDir, gitBin: this.#git });
    const object = await batch.read(info.objectId);
    if (object.type !== "blob") throw new Error(`Expected blob for ${p}, got ${object.type}`);
    return object.bytes;
  }
  ...
  async *listFiles(): AsyncIterable<string> {
    ...
    for (const p of (stdout as Buffer).toString("utf8").split("\0")) {
      if (p && this.#relevant(p)) yield p;
    }
  }
  ...
  async #ensureRepo(): Promise<void> {
    ...
    let exists = true;
    try { await access(this.#gitDir); } catch { exists = false; }
    if (!exists) await this.#runRaw(["init", "--bare", this.#gitDir]);
  ...
  async #ensureCommit(): Promise<void> {
    if (!this.#commit) await this.#ensureRepo();
  }

  #relevant(path: string): boolean {
    if (this.#sparse.length === 0 || path === "") return true;
    return this.#sparse.some((prefix) => path === prefix || path.startsWith(`${prefix}/`) || prefix.startsWith(`${path}/`));
  }

  #run(args: string[], options: { encoding?: "buffer"; maxBuffer?: number } = {}): Promise<{ stdout: string | Buffer; stderr: string | Buffer }> {
    return this.#runRaw([`--git-dir=${this.#gitDir}`, ...args], options);
  }

  async #runRaw(args: string[], options: { encoding?: "buffer"; maxBuffer?: number } = {}): Promise<{ stdout: string | Buffer; stderr: string | Buffer }> {
...
function rejectOptionLike(field: "ref" | "remote", value: string): void {
  if (value.startsWith("-")) {
    throw new Error(`Invalid git ${field} (must not start with '-'): ${value}`);
  }
}

function isMissingTreeish(error: unknown): boolean {
