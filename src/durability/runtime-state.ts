 ...
 */
export function canReplaceEffect(existing: DurableEffectRecord | undefined, next: DurableEffectRecord): boolean {
  if (!existing) return true;
  if (existing.status === "committed" && next.status !== "committed") return false;
  if (existing.status === "failed" && next.status === "started") return false;
  return true;
}
