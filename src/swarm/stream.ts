...
export const FINDING_KINDS: readonly FindingKind[] = ["incident", "slow-burn", "correlation"];
...
export const PLANTED_STREAM: SignalStream = {
...
export function streamToJsonl(stream: SignalStream = PLANTED_STREAM): string {
  return `${stream.events.map((event) => JSON.stringify(event)).join("\n")}\n`;
}

export function parseStreamJsonl(text: string): StreamEvent[] {
  return text
    .split("\n")
    .filter(Boolean)
...
export function groundTruthEventIds(stream: SignalStream = PLANTED_STREAM): Set<string> {
  const ids = new Set<string>();
  for (const entry of [...stream.planted, ...stream.ambiguous, ...stream.decoys]) {
    for (const id of entry.evidence) ids.add(id);
  }
  return ids;
}
