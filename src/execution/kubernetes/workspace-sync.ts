...
export class WorkspaceSynchronizer {
  ...
  readonly #initializeGitBaseline: boolean;
  ...
  async materialize(workspace: MemoryWorkspace, sandbox: SandboxIdentity): Promise<void> {
    let files = 0;
    let bytes = 0;
    ...
    if (this.#initializeGitBaseline) {
      const init = await this.#backend.exec(sandbox, {
        command:
          "git init -q && git config user.name synth-runtime && git config user.email synth@invalid && git add -A && git commit -q --allow-empty -m synthetic-base",
        timeoutMs: 120_000,
  ...
  #checkLimits(files: number, bytes: number, phase: string): void {
    if (files > this.#maxFiles) throw new Error(`${phase} exceeds sync file limit (${this.#maxFiles})`);
    if (bytes > this.#maxBytes) throw new Error(`${phase} exceeds sync byte limit (${this.#maxBytes})`);
  }
}
