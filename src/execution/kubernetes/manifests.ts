...
export function assertValidNamespace(namespace: string): string {
  if (namespace.length === 0 || namespace.length > 63 || !DNS1123_LABEL.test(namespace)) {
    throw new Error(
      `Invalid Kubernetes namespace name: ${JSON.stringify(namespace)} (expected a DNS-1123 label of at most 63 characters)`,
    );
  }
  return namespace;
}

export function sandboxLabels(id: string, resourceClass: KubernetesResourceClass): Record<string, string> {
  return {
    [LABEL_MANAGED]: "true",
...
function selectorExpression(labels: Record<string, string> | undefined): Record<string, unknown> {
...
export function buildRestrictedNamespace(namespace: string, labels: Record<string, string> = {}): KubernetesObject {
  return {
    apiVersion: "v1",
    kind: "Namespace",
    metadata: {
...
export function buildProjectServicePod(
  namespace: string,
  cellId: string,
...
export function buildProjectService(namespace: string, cellId: string, service: ProjectCellService): KubernetesObject | undefined {
  if (!service.ports?.length) return undefined;
  const name = `cell-${cellId}-${service.name}`.toLowerCase().replace(/[^a-z0-9-]/g, "-").slice(0, 63);
  return {
    apiVersion: "v1",
    kind: "Service",
...
export function buildProjectCellNetworkPolicy(namespace: string, cellId: string): KubernetesObject {
  return {
    apiVersion: "networking.k8s.io/v1",
    kind: "NetworkPolicy",
