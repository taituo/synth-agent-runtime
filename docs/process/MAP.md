...
`grok-4.6`, and so on. 27 are available on the current subscription.
...
`listModels()` and `handle(request, model)`. Implementations:
...
**Profile** — one routing entry in the router: a model plus the backend that serves it, plus
its health state. A profile is NOT "an account" — that was a narrow reading. Profiles can
span providers, so routing across providers needs no new accounts.

### The visibility trap that caused real confusion
The probe host `/tmp/opencode/audit/pi/ogw-host.mts` passes
`modelIds: ["muse-spark-1.3-contributor"]`. That option is a FILTER, not a definition
(`adapter.ts:54` turns it into an `allowed` set). One hardcoded line made 27 models look like
one, and every accuracy and latency number recorded so far is that single model's number.
...
- **synthetic** (`src/execution/synthetic.ts`): an in-memory workspace, no filesystem,
  fidelity 0. Fast and free. `process.exec` is not possible here, so it escalates.
- **real** (`src/execution/kubernetes/`): a gVisor sandbox with a real filesystem and real
  processes.

**ExecutionBroker** picks the rung PER EFFECT and escalates when needed, so a single turn can
be part in-memory and part sandboxed. Receipts record which rung ran what.
...
by a 4 MB artifact moving workflow history by 17 bytes.
