...
A transient failure may carry a **server retry hint**. On a 429/503 the gateway activity parses `Retry-After` (seconds or HTTP-date) and the `*-ratelimit-reset` spellings, using the response's own `Date` header as the reference clock so worker clock skew cannot produce a bad wait, and attaches `retryAfterMs` to the failure. The workflow waits for exactly that duration instead of the exponential backoff, provided the hint is finite, positive and at most one hour (anything else — absent, negative, absurd — falls back to the backoff, so a hostile upstream cannot park an agent for a week). A hint beyond the one-hour clamp is not ordinary throttling — it means the account is out of quota for the window — so the agent's `lastError` is prefixed `QUOTA_EXHAUSTED:` and the park log uses `reason: "quota-exhausted"`, making the condition visible in traces rather than looking like a normal retry. (`402` is likewise a permanent failure: credit/plan exhaustion never succeeds on retry.) A hinted wait deliberately does **not** escalate `parkAttempt`: the server told us when to return, so returning then is not a failed attempt. This matters because the runtime runs on subscription accounts, where quota-in-a-window is the scarce resource: blind backoff either burns quota on requests certain to fail or sleeps past the reset it was waiting for.

An activity may also **defer** a turn by returning `state: "waiting"` without consuming its messages (the type allows it). That is treated as the same park as a transient failure: the mailbox is kept intact, the workflow backs off exponentially, and the turn is retried. This is deliberate — a deferring activity must never busy-loop the workflow. Before this, a `waiting` return left the mailbox non-empty, so the loop's wake condition was immediately true and the same turn re-ran with zero delay; measured live at ~12 activity calls/second with the mailbox stuck at length 1, growing history until Temporal killed the workflow. The alternative (declaring `waiting` illegal for an activity and rejecting it loudly) was rejected because the contract already models `waiting` as a legitimate durable state and the park path gives it safe, bounded semantics for free.
...
Temporal retries an activity whose result was never recorded (worker death, heartbeat timeout). An activity must therefore be idempotent: give each external side effect a stable idempotency key and let the effect receipt decide. `ExecutionBroker` claims an effect by `effect.id`; the shipped rung persists the receipts in Temporal activity state (`TemporalActivityStateStore`, carried in the heartbeat details), so the retry starts with the committed receipts and does not re-execute a committed effect. A `started` receipt left by a dead worker is returned as `EFFECT_OUTCOME_UNCERTAIN`, not blindly replayed. Timeout alone is not permission to repeat an external action. Live: `effect-receipt-live.ts` (attempt 1 `write_file:0` committed, attempt 2 seeds it committed and executes it once).
