...
- Added expiring leases with monotonically increasing fencing tokens and renewable lease execution.
- Added `CommandCoordinator` with durable owner/fence metadata and explicit abandoned-command reconciliation.
- Added durable mailbox sequence + named ACK cursor semantics and integrated the engine cursor into `AgentRuntime`.
- Added `ProjectSpec.revision` and compare-and-swap project updates across memory, JSON-file, and PostgreSQL worlds.
- Added `EffectReconciler` for fail-closed uncertain external-effect resolution.
- Added shared Responses continuation and router health/affinity store interfaces, with tenant isolation and PostgreSQL implementation.
- Added gateway authentication/model ACL/tenant identity propagation and reference rate limiting.
- Added resumable sequenced runtime-event reads and retention pruning.
- Added `PostgresDistributedControlStore` and `deploy/postgres/002_distributed_control_plane.sql`.
- Expanded the live PostgreSQL concurrency scenario with lease and project-CAS contention.
- Added `test/v08.test.ts`; full local suite is 62 passing tests.
- Preserved the original v0.7 root documentation under `docs/history/synth-agent-runtime-v0.7/`.
- Added `DISTRIBUTED.md` and refreshed current architecture, recovery, Postgres, inference, hardening, roadmap, upgrade, and review documents.
