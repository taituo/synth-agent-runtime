...
import { AgentRuntime, InMemoryMailboxStore, LocalMemoryDurability, MemoryWorkspace, type AgentEngine, type AgentId } from "../src/index.js";
...
test("same agent runs unattended and can be observed interactively", async () => {
  ...
  const engine: AgentEngine = { async run() { ws.write("a.txt", "hello"); return "ok"; } };
...
test("runtime serializes streamed output events before completed state", async () => {
  class DelayedDurability extends LocalMemoryDurability {
    override async appendEvent(event: Parameters<LocalMemoryDurability["appendEvent"]>[0]): Promise<void> {
      if (event.type === "agent.output") await new Promise((resolve) => setTimeout(resolve, 25));
...
test("concurrent duplicate spawns across runtime replicas create one durable identity", async () => {
  ...
  const engine: AgentEngine = { async run() { return "ok"; } };
  const id = "replica-race" as AgentId;
...
test("durable mailbox dedup prevents duplicate steer across runtime replicas", async () => {
  ...
  const firstSteers: string[] = [];
  const secondSteers: string[] = [];
  const firstEngine: AgentEngine = { async run() { return "ok"; }, async steer(message) { firstSteers.push(message.id); } };
  const secondEngine: AgentEngine = { async run() { return "ok"; }, async steer(message) { secondSteers.push(message.id); } };
