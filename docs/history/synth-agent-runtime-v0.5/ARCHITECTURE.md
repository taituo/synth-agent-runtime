...
v0.4 explicitly separates canonical state from live process objects:
...
Warm sandboxes are reset before reuse. v0.4 adds an optional post-reset proof; failed verification destroys the sandbox instead of returning it to the pool.
...
## v0.5 multi-process persistence boundary
