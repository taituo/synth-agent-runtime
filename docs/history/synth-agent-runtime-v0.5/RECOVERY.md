...
## Multi-process claims in v0.5

Recovery alone does not prevent two live workers from racing. `RuntimeStateStore` therefore gained optional atomic claim operations. `PostgresPersistence` implements them with conflict-aware inserts, and runtime/broker code automatically uses them when present. Local/JSON reference stores remain suitable for single-process use.
