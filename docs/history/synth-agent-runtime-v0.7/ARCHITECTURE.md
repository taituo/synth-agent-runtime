...
The runtime explicitly separates canonical state from live process objects:
...
Warm sandboxes are reset before reuse. Post-reset verification can prove the workspace/temp boundary is clean; failed verification destroys the sandbox instead of returning it to the pool. v0.7 also fixes the concurrent create/waiter path so one slot cannot be double-leased.
...
## Multi-process persistence boundary
...
## v0.7 reviewed transaction invariant

Attempt-local effect receipts are scoped to the durable turn so a rolled-back provider attempt cannot reuse a receipt from a different workspace state. Barrier/commit effects persist `semanticExposed` before crossing the external boundary. A process crash after that boundary recovers to an explicit reconciliation-required failure rather than a transparent retry.

## Remaining distributed-state seams

The architecture is intentionally explicit about the parts that are still per-process or last-writer-wins: Responses continuation cache, route affinity/health, mailbox consumption, event scans, and whole-document project updates. `CODE-REVIEW.md` defines these as v0.8 correctness work.
