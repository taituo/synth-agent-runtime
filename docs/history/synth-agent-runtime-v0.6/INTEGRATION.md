...
Use `PostgresPersistence` directly with any structural `PgExecutor`, or use `integrations/postgres/node-pg.ts` for `pg.Pool`. The same instance may be passed as durability, runtime-state, and world storage.

### Pi E2E

`integrations/pi-e2e/install-test.sh` installs a real `AgentHarness` tool-turn test into a Pi checkout. It validates the same `ExecutionEnv` injection seam used by the synthetic runtime.

### Chaos

Wrap stores/executors/gateways with `Chaos*` wrappers and use named failpoints. This is intended for deterministic regression tests as well as higher-level process-kill scenarios.
