...
## v0.8 distributed note

Transactional turn semantics are unchanged. Multi-replica command ownership now belongs to `CommandCoordinator`; uncertain effects can be resolved through `EffectReconciler`. A lease/fence does not make an external effect replay-safe by itself.
