...
function trackingReal(order: string[]): Executor {
  return {
    id: "fake-real",
    fidelity: 1,
...
function context(workspaceId: WorkspaceId): EffectContext {
  return { agentId: "agt_mixed_unit" as EffectContext["agentId"], workspaceId };
}
...
test("mixed chain preserves effect order across the rung boundary", async () => {
  ...
  const results = [];
  for (const effect of chain) results.push(await broker.execute(effect, ctx));
...
test("a mixed chain records executor and fidelity per effect; overall is the lowest rung", async () => {
  ...
  assert.equal((first?.result as { executor?: string }).executor, "synthetic");
  assert.equal((first?.result as { fidelity?: number }).fidelity, 0);
  assert.equal((second?.result as { executor?: string }).executor, "fake-real");
  assert.equal((second?.result as { fidelity?: number }).fidelity, 1);
  const overall = Math.min((first?.result as { fidelity?: number }).fidelity!, (second?.result as { fidelity?: number }).fidelity!);
