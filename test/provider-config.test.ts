...
async function fakeProvider(id: string, options: { status?: number } = {}): Promise<{ url: string; calls: () => number; close: () => Promise<void> }> {
  let calls = 0;
...
function postCompletion(baseUrl: string, model: string): Promise<Response> {
...
test("config drives provider construction and routing selects the configured provider", async () => {
  ...
  try {
    ...
    try {
      const a = await (await postCompletion(gateway.url, "alpha")).json() as { model: string; echoedModel: string };
      ...
      const b = await (await postCompletion(gateway.url, "beta")).json() as { model: string; echoedModel: string };
...
test("opencode-go is one profile among many, selected by config", async () => {
  ...
  try {
    ...
    await gateway.listen();
    try {
      assert.equal(((await (await postCompletion(gateway.url, "opencode-go")).json()) as { model: string }).model, "opencode-go");
      assert.equal(((await (await postCompletion(gateway.url, "other")).json()) as { model: string }).model, "other");
    } finally { await gateway.close(); }
  } finally { await opencodeGo.close(); await other.close(); }
});
...
test("providers sharing a profile fail over in config order", async () => {
  ...
  try {
    ...
    await gateway.listen();
    try {
      const served = await (await postCompletion(gateway.url, "cheap")).json() as { model: string };
  ...
  } finally { await down.close(); await up.close(); }
});
...
test("a provider is swapped in by config alone", async () => {
  ...
  try {
    ...
    await gateway.listen();
    try {
      const served = await (await postCompletion(gateway.url, "alpha")).json() as { model: string };
  ...
  } finally { await alpha.close(); await gamma.close(); }
});
...
test("a synthetic run reaches a declared provider directly (no gateway, no opencode)", async () => {
  ...
  try {
    const provider: ProviderConfig = { id: "delta", baseUrl: delta.url, model: "delta-upstream" };
    ...
    const context: AgentEngineContext = {
      agentId: "agt_provider" as AgentId,
      workspaceId: "ws_provider" as WorkspaceId,
  ...
  } finally { await delta.close(); }
});

test("providers come from the environment with no hardcoded provider or key", () => {
  assert.deepEqual(
    providersFromEnv({ SYNTH_GATEWAY_PROVIDERS: JSON.stringify([{ id: "x", baseUrl: "http://x", model: "m" }]) }).map((p) => p.id),
    ["x"],
  );
  assert.deepEqual(
    providersFromEnv({ SYNTH_PROVIDER_ALPHA_BASEURL: "http://a", SYNTH_PROVIDER_ALPHA_MODEL: "am", SYNTH_PROVIDER_ALPHA_API_KEY: "k" }),
    [{ id: "alpha", baseUrl: "http://a", model: "am", apiKey: "k" }],
  );
