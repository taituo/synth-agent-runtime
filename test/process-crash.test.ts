...
test("SIGKILL during a durable turn is recovered from persisted state", async () => {
  ...
  try {
    ...
    const definition: AgentDefinition = { id: "crash-worker", inferenceProfile: { id: "test" } };
    ...
    const agent = runtime.get(ready.agentId as any);
    ...
    const workspace = runtime.workspaces.get(ready.workspaceId as any);
    assert.ok(workspace);
    assert.equal(await workspace.readText("dirty-before-crash.txt"), undefined);

    const checkpoint = await state.getWorkspaceCheckpoint(ready.workspaceId as any);
...
function waitForReady(child: ReturnType<typeof spawn>): Promise<ReadyMessage> {
