...
async function withSource<T>(cache: string, repo: RealRepo, fn: (source: NativeGitSource) => Promise<T>): Promise<T> {
...
test("every pinned repo exposes exactly its tree at the pinned commit", async (t) => {
  for (const repo of REAL_REPOS) {
    ...
    await withSource(cache, repo, async (source) => {
      ...
      const listed: string[] = [];
      for await (const path of source.listFiles()) listed.push(path);
...
test("opening the same pinned revision twice is idempotent", async (t) => {
  ...
  const first = await withSource(cache, repo, async (source) => {
    const revision = await source.revision();
    const files: string[] = [];
    for await (const path of source.listFiles()) files.push(path);
...
test("a cold cache with an unreachable remote raises FixtureUnavailableError (skip path)", async () => {
  ...
  try {
    await assert.rejects(
      () => repoCachePath({ name: "missing", url: join(dir, "does-not-exist"), commit: "0".repeat(40), license: "n/a" }),
      (error: unknown) => {
        assert.ok(error instanceof FixtureUnavailableError, "must be a skippable fixture error");
        assert.match((error as Error).message, /cache is cold and clone failed/);
        return true;
      },
    );
  } finally {
    if (previous === undefined) delete process.env.SYNTH_FIXTURE_REPOS;
    else process.env.SYNTH_FIXTURE_REPOS = previous;
