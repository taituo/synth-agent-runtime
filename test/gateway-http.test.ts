...
test("HTTP gateway exposes dynamic port, models, and Responses transport", async () => {
  ...
  try {
    ...
    const models = await fetch(`${gateway.url}/v1/models`).then((response) => response.json()) as any;
...
test("profile router uses Responses metadata.session_id for sticky routing", async () => {
  const calls: string[] = [];
  const backend = (id: string, firstStatus = 200): GatewayBackend => ({
...
test("profile router scopes route health by virtual model", async () => {
  let failingCalls = 0;
  const backend = (status: number): GatewayBackend => ({
    async listModels() { return []; },
    async handle() { if (status === 429) failingCalls++; return new Response("{}", { status }); },
  });
  const router = new ProfileRouterBackend({
  ...
  const req = (model: string) => new Request("http://router/v1/responses", {
    method: "POST",
    headers: { "content-type": "application/json" },
