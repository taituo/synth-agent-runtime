...
export const REAL_REPOS: readonly RealRepo[] = [
...
export async function repoCachePath(repo: RealRepo): Promise<string> {
  ...
  try {
    ...
    throw new FixtureUnavailableError(
      `fixture repo ${repo.name} cache is cold and clone failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  return dir;
}
...
export async function listTreeFiles(cachePath: string, commit: string): Promise<string[]> {
...
export async function blobBytes(cachePath: string, commit: string, path: string): Promise<Buffer> {
