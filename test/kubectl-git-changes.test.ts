...
function shSupportsReadD(): boolean {
...
test("the git-change loop works under a dash outer shell (the repo executor's /bin/sh)", async (t: TestContext) => {
  ...
  try {
    ...
    assert.deepEqual(
      fixedRecords.sort(),
      ["dir with space/naïve--name.txt", "new.txt"],
      `gitChangesCommand must report both changes; stdout=${JSON.stringify(fixed.stdout)} stderr=${fixed.stderr}`,
    );
    ...
    const oldStyle = spawnSync(
      "sh",
      ["-c", `cd ${JSON.stringify(repo)} && git status --porcelain=v1 -z --untracked-files=all --no-renames | sh -c "$${GIT_CHANGES_LOOP_ENV}"`],
      { env, encoding: "utf8" },
    );
    assert.equal(
      records(oldStyle.stdout).length,
      0,
      `the un-delegated loop must emit nothing under dash; stdout=${JSON.stringify(oldStyle.stdout)} stderr=${JSON.stringify(oldStyle.stderr)}`,
    );
