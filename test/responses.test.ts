...
test("Responses object preserves text, function calls and usage", () => {
  ...
  assert.equal((response.output[1] as any).call_id, "call_1");
...
test("Responses object and stream preserve reasoning summaries", () => {
  ...
  assert.equal((response.output[0] as any).type, "reasoning");
  assert.equal((response.output[0] as any).summary[0].text, "Checked the constraints.");
