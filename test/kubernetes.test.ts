...
test("the NetworkPolicy is owned by its Pod so Kubernetes reaps it with the pod", () => {
  ...
  const owned = buildSandboxNetworkPolicy("test", "network", "sandbox", cls.network, owner) as any;
  assert.deepEqual(owned.metadata.ownerReferences, [owner]);
  // Callers that pass no owner keep the previous manifest shape.
  const unowned = buildSandboxNetworkPolicy("test", "network", "sandbox", cls.network) as any;
  assert.equal(unowned.metadata.ownerReferences, undefined);
});

/** The name a per-sandbox policy always has. */
const policyName = (podName: string): string => `${podName}-network`;
...
test(
  "cleanup is a property: no per-pod NetworkPolicy is left behind",
  { skip: liveKubernetes ? false : "set SYNTH_LIVE_GVISOR=1 (and SYNTH_KUBERNETES_NAMESPACE) for the live cleanup proof" },
  async () => {
    const kubectl = (...args: string[]): string => execFileSync("kubectl", args, { encoding: "utf8" });
    const exists = (resource: string, name: string): boolean => {
      try {
    ...
    const waitGone = async (resource: string, name: string, tries = 30): Promise<boolean> => {
      for (let i = 0; i < tries; i++) {
        if (!exists(resource, name)) return true;
        await sleep(2_000);
      }
    ...
    if (!policyGone) kubectl("delete", "netpol", policy, "-n", liveNamespace, "--ignore-not-found=true", "--wait=false");
