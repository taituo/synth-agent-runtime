...
test("the scored-rung gate has two refusals and one allow (A/B/C)", () => {
  ...
  // blanket refusal.
  assert.doesNotThrow(() => assertScoredRunnerAllowed("sandbox"));
  assert.equal(describeGymRunner("sandbox").scoredAllowed, true);
...
test("materializing a task does not plant the hidden vectors or the hidden test", async (t: TestContext) => {
  ...
  try {
    ...
    assert.ok(
      found.some((path) => path.endsWith(task.visibleTestPath) || path === task.visibleTestPath),
      `the visible test must be planted; saw ${found.join(", ")}`,
    );
    assert.ok(
      !found.some((path) => path.includes(HIDDEN_CASES_FIXTURE)),
      `hidden.cases.json must never reach the checkout; saw ${found.join(", ")}`,
    );
    assert.ok(
      !found.some((path) => path.includes("hidden.test.mjs")),
      `the hidden test must never reach the checkout; saw ${found.join(", ")}`,
    );
    assert.ok(
      !existsSync(join(materialized.repoDir, "test", HIDDEN_CASES_FIXTURE)),
      "hidden.cases.json must not exist in the checkout",
    );
  } catch (error) {
