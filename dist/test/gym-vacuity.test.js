...
import { WORKER_SOURCE, isolatedScoreGymPatch, scoreGymPatch } from "../src/index.js";
...
test("FORGE 1: reading GYM_HIDDEN_NONCE and printing a completion marker does not pass", async () => {
    ...
    try {
        ...
        assert.ok(score.cases.some((entry) => /no-nonce/.test(entry.error ?? "")), "the child's environment must not carry GYM_HIDDEN_NONCE");
    }
    finally {
...
test("the worker runs one-shot in batch mode, so a sandboxed exec needs no fd", async () => {
    ...
    try {
        ...
        await writeFile(requests, JSON.stringify([
