...
import { isolatedScoreGymPatch, scoreGymPatch } from "../src/index.js";
...
async function makeRepo(parent, files) {
...
async function assertAllDenied(parent, source, calls, context) {
    const repo = await makeRepo(parent, { "probe.mjs": source });
    const cases = calls.map((call) => ({ module: "./probe.mjs", call, args: [], expect: "denied", label: call }));
    ...
    for (const result of score.cases)
        assert.ok(result.ok, `${context}: ${result.label}: ${result.error}`);
    return score.cases;
}
...
test("CHANNELS fs: absolute, /proc, /etc, the fixture tree and the scorer source are all unreadable", async () => {
    ...
    try {
        ...
        await assertAllDenied(parent, source, [
            "readAbsolute",
            "readProcSelfEnviron",
            "readProcSelfMaps",
            "readProcSelfCwd",
...
test("CHANNELS builtins: process.binding, worker_threads, module.register and node:sqlite are denied", async () => {
    ...
    try {
        ...
        await assertAllDenied(parent, source, ["bindingFs", "bindingSpawnSync", "bindingTcpWrap", "workerThreads", "moduleRegister", "sqlite", "sqliteGetBuiltin"], "builtins");
    }
    finally {
        await rm(parent, { recursive: true, force: true });
    }
...
async function symlinkPatch(repo, plant) {
