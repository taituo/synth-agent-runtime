...
async function runDriver(script, env) {
    try {
...
const DRIVERS = ["corpus-model-compare.ts", "rate-limit-scope.ts"];
for (const driver of DRIVERS) {
    test(`${driver}: no MODELS is a skip with exit 2`, async (t) => {
        if (!(await tsxAvailable()))
            return t.skip("tsx not installed");
