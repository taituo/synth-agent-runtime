...
import { AgentRuntime, DurableTurn, ExecutionBroker, JsonFileRuntimeStateStore, LocalMemoryDurability, LocalRuntimeStateStore, MemoryWorkspace, ProfileRouterBackend, SyntheticExecutor, runDurableTransactionalTurn, } from "../src/index.js";
...
test("barrier effect prevents transparent retry", async () => {
    ...
    const store = new LocalRuntimeStateStore();
    let approvals = 0;
    await assert.rejects(runDurableTransactionalTurn({
        workspace,
        store,
...
test("attempt-local effect receipts are scoped per durable retry attempt", async () => {
    ...
    const context = { agentId: "agent-retry", workspaceId: workspace.id };
    let attempts = 0;
    const result = await runDurableTransactionalTurn({
        workspace,
        store: state,
