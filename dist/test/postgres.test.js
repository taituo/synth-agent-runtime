...
import { canReplaceEffect, ExecutionBroker, installPostgresSchema, PostgresPersistence, PostgresRateLimitStore, SharedTenantRateLimitPolicy, } from "../src/index.js";
class FakePg {
    commands = new Map();
    effects = new Map();
    rateLimits = new Map();
    ...
    async query(text, values = []) {
        ...
        if (sql.startsWith("WITH max_seq AS") && sql.includes("INSERT INTO synth_event_cursors")) {
            ...
            return { rows: [{ ack_seq: ack, updated_at_ms: row.updated_at_ms }] };
        }
        ...
        if (sql.startsWith("DELETE FROM synth_rate_limits")) {
            ...
            return { rows: Array.from({ length: removed }, () => ({ tenant_id: "t" })) };
        }
...
test("Postgres atomic command claim allows only one active owner", async () => {
    ...
    const record = { id: "cmd-1", status: "started", startedAt: 1, updatedAt: 1 };
...
test("Postgres effect claim prevents duplicate executor calls", async () => {
    ...
    const effect = { id: "effect-1", kind: "process.exec", command: "true" };
...
test("Postgres rate limit store shares one counter across store instances", async () => {
    ...
    assert.deepEqual([await attempt(policyA), await attempt(policyB), await attempt(policyA)], ["ok", "ok", "limited"]);
