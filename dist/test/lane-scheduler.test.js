...
import { LaneScheduler } from "../src/index.js";
const LANES = [
...
function request(lane, key = lane) {
...
test("queues with a ticket once full, and lists it as pending", () => {
    ...
    assert.equal(typeof decision.ticket, "string");
...
test("a single lane with capacity behaves like the old path under capacity", () => {
    ...
    for (let i = 0; i < 3; i++)
        assert.deepEqual(scheduler.admit(request("default", `k${i}`)), { outcome: "admit" });
...
test("weighted fair-share within a band tracks the configured weights", () => {
    ...
    const admitted = { heavy: 0, light: 0 };
    for (let i = 0; i < 400; i++) {
...
test("a queued request reports an estimated Retry-After", () => {
    ...
    assert.equal(first.retryAfterMs, 0, "nothing queued ahead");
    const second = scheduler.admit(request("batch", "q2"));
    assert.equal(second.retryAfterMs, 1_000, "one request ahead");
});
...
test("property: release() never admits a request past its lane deadline", () => {
    ...
    for (let trial = 0; trial < 300; trial++) {
        ...
        const laneOfTicket = new Map();
        const check = (released) => {
            const spec = laneOfTicket.get(released.ticket);
            const waited = now - enqueuedAt.get(released.ticket);
            assert.ok(waited <= spec.maxWaitMs, `trial ${trial}: ${released.ticket} waited ${waited}ms, lane allows ${spec.maxWaitMs}ms`);
        };
...
test("a lower band is reserved a share, so sustained high-band load cannot starve it", () => {
    ...
    for (let i = 0; i < 50; i++)
        scheduler.admit(request("high", `h${i}`)); // high backlog never drains
    ...
    for (let i = 0; i < 100 && lowAdmittedAt < 0; i++) {
        ...
        if (!next)
            break;
        if (next.request.lane === "low")
            lowAdmittedAt = i;
        else
            scheduler.admit(request("high", `keep-${i}`)); // keep the high band saturated
    }
