...
import { AgentRuntime, InMemoryLeaseStore, LeasedAgentRunner, LocalMemoryDurability, MemoryWorkspace, PostgresDistributedControlStore, PostgresPersistence, } from "../src/index.js";
const fence = (agentId, ownerId, fencingToken) => ({
...
class RecordingPg {
    ...
    async query(text, values = []) {
        ...
        if (sql.includes("INSERT INTO synth_leases")) {
            return { rows: [{ resource_id: values[0], owner_id: values[1], fencing_token: 7, acquired_at_ms: 1000, updated_at_ms: 1000, expires_at_ms: 6000 }] };
        }
        if (sql.includes("UPDATE synth_leases SET") && sql.includes("expires_at_ms=db_clock.now_ms+$4")) {
            return { rows: [{ resource_id: values[0], owner_id: values[1], fencing_token: values[2], acquired_at_ms: 1000, updated_at_ms: 2000, expires_at_ms: 7000 }] };
        }
        if (sql.includes("FROM synth_leases, db_clock") && sql.includes("expires_at_ms>db_clock.now_ms")) {
            if (sql.includes("INSERT INTO synth_agents"))
