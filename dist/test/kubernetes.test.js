...
class MockSandboxBackend {
    ...
    async reset() { this.resets++; this.files.clear(); this.baseline.clear(); this.links.clear(); this.baselineLinks.clear(); }
    ...
    async removePath(_sandbox, path) { this.files.delete(path); this.links.delete(path); }
    async listGitChanges() {
        ...
        for (const path of [...paths].sort()) {
            ...
            const sameEntry = baselineLink !== undefined || currentLink !== undefined
...
test("workspace sync preserves symlinks in both directions", async () => {
    const backend = new MockSandboxBackend();
    const sandbox = { id: "s", namespace: "n", podName: "p", resourceClassId: "sandbox-small", createdAt: 1 };
    const workspace = new MemoryWorkspace();
...
class FakeProjectCellController {
    applied = [];
    deleted = [];
    ...
    async waitPodReady(_namespace, podName) {
        if (this.delayMs)
            await new Promise((resolve) => setTimeout(resolve, this.delayMs));
        if (this.failWait)
            throw new Error(`Pod ${podName} not ready`);
    }
...
test("project service pod omits user/group overrides when not requested", () => {
    const pod = buildProjectServicePod("test", "cell-1", { name: "db", image: "postgres:16-alpine" });
...
test("project cell ensure rolls back a half-built cell when a service never becomes ready", async () => {
    ...
    const backend = new MockSandboxBackend();
    const manager = new ProjectCellManager(controller, backend, [projectCellClass()]);
    await assert.rejects(manager.ensure({ id: "cell-1", services: [{ name: "db", image: "postgres:16-alpine" }] }), /not ready/);
...
test("project cell ensure does not delete a caller-provided namespace on failure", async () => {
    ...
    const backend = new MockSandboxBackend();
    const manager = new ProjectCellManager(controller, backend, [projectCellClass()]);
    await assert.rejects(manager.ensure({ id: "cell-1", namespace: "shared-ns", services: [{ name: "db", image: "postgres:16-alpine" }] }), /not ready/);
...
test("project cell reap cannot delete a namespace a concurrent lease just recreated", async () => {
    ...
    await reaping;
    assert.ok(controller.activeNamespaces.has(lease.handle.namespace), "the leased cell's namespace must still exist after a concurrent reap");
});
...
test("sandbox exec marks the control-plane workspace safe for git", async () => {
    ...
    try {
        ...
        const sandbox = { id: "s", namespace: "synth-test", podName: "pod", resourceClassId: "sandbox-small", createdAt: 0 };
