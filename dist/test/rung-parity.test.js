...
import { MemoryWorkspace, NativeGitSource, SyntheticExecutor, WORKSPACE_NOT_DIRECTORY, WORKSPACE_NOT_FOUND, WORKSPACE_PATH_ESCAPES, } from "../src/index.js";
...
function context(workspaceId) {
    return { agentId: "agt", workspaceId };
}
async function compare(effects, seed) {
...
test("../ traversal: synthetic rejects, the raw OS escapes (documented divergence)", async () => {
    ...
    try {
        const effect = { id: "1", kind: "workspace.write", path: "../escape.txt", content: "x" };
        const { executor, workspace } = synthetic();
        const ctx = context(workspace.id);
...
test("absolute paths are rejected, not silently rewritten", async () => {
    ...
    try {
        ...
        const effect = { id: "1", kind: "workspace.write", path: absolute, content: "x" };
        const { executor, workspace } = synthetic();
        const ctx = context(workspace.id);
        const synth = await executor.execute(effect, ctx);
        assert.equal(synth.ok, false);
...
test("a source-backed symlink read follows the link, matching the independent oracle", async (t) => {
    ...
    try {
        ...
        const oracle = await new RealFsOracle(oracleRoot).execute({ id: "r", kind: "workspace.read", path: link }, { agentId: "a", workspaceId: workspace.id });
        assert.equal(oracle.ok, true);
        assert.ok(Buffer.from(oracle.output).equals(Buffer.from(synthetic)), "synthetic and oracle reads agree");
    }
    finally {
        await source.close();
        await rm(parent, { recursive: true, force: true });
...
test("diffOutcomes exempts a divergence by path shape, not by the synthetic error string", () => {
    const inWorkspace = diffOutcomes([{ id: "1", kind: "workspace.read", path: "a.txt", ok: true, category: "", outputBytes: "x" }], [{ id: "1", kind: "workspace.read", path: "a.txt", ok: false, category: "denied", error: "WORKSPACE_PATH_ESCAPES:a.txt" }]);
    assert.equal(inWorkspace[0].kind, "other", "an in-workspace path is never exempt");
    const escaping = diffOutcomes([{ id: "2", kind: "workspace.write", path: "../x", ok: true, category: "", outputBytes: "" }], [{ id: "2", kind: "workspace.write", path: "../x", ok: false, category: "denied", error: "WORKSPACE_PATH_ESCAPES:../x" }]);
