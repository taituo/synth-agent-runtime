...
import { CompositeTenantPolicy, LaneScheduler, PriorityLanePolicy, } from "../src/index.js";
const LANES = [
    { id: "interactive", priority: 10, weight: 1, maxWaitMs: 5_000 },
    { id: "batch", priority: 1, weight: 1, maxWaitMs: 5_000 },
];
function principal(subject, lane) {
...
test("a request past its lane deadline rejects with a Retry-After", async () => {
    ...
    await assert.rejects(policy.authorize(principal("q", "short")), (error) => {
        assert.match(String(error), /LANE_DEADLINE/);
        assert.equal(error.retryAfterMs, 20);
        return true;
    });
});
