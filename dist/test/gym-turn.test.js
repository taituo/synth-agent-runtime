...
test("the turn retries transient HTTP failures up to maxAttempts, then succeeds", async () => {
    ...
    const turn = createGatewayGymTurn({
        ...
        fetchImpl: (async () => {
            ...
            return new Response(JSON.stringify({ model: "wanted", choices: [{ message: { content: JSON.stringify({ tool_calls: [{ name: "finish" }] }) } }] }), { status: 200, headers: { "content-type": "application/json" } });
        }),
    });
    const result = await turn(INPUT);
...
test("a retried 429 honours the Retry-After hint", async () => {
    ...
    const turn = createGatewayGymTurn({
        ...
        fetchImpl: (async () => {
            ...
            return new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ tool_calls: [{ name: "finish" }] }) } }] }), { status: 200, headers: { "content-type": "application/json" } });
        }),
    });
    await turn(INPUT);
