...
import { createInferenceGateway, litellmProfile, ProfileRouterBackend, } from "../src/index.js";
function startEndpoint(behavior, marker) {
...
test("a healthy primary is used; a down primary fails over and records cooldown", async () => {
    ...
    const backends = {
        [controlPrimary.backendName]: controlPrimary.backend,
        [controlFallback.backendName]: controlFallback.backend,
        [downPrimary.backendName]: downPrimary.backend,
        [failoverFallback.backendName]: failoverFallback.backend,
    };
