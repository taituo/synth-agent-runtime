...
test("both drivers start the durable gymAttemptWorkflow through the Temporal client", () => {
    for (const [name, source] of [["run-gym.ts", RUN_GYM], ["p2-faults.ts", P2_FAULTS]]) {
...
test("the gym workflow owns its loop and does not drive durableAgentWorkflow (gym-7 decision)", () => {
    ...
    assert.ok(!/startChild|executeChild|getChildWorkflowHandle|ChildWorkflow/.test(GYM_WORKFLOWS), "the gym workflow must not start a child workflow");
