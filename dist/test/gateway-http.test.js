...
test("a 413 does not poison a reused keep-alive connection", async () => {
    ...
    const post = (body) => new Promise((resolve) => {
        const req = http.request({ hostname: base.hostname, port: base.port, path: "/v1/chat/completions", method: "POST", agent, headers: { "content-type": "application/json" } }, (res) => { res.resume(); res.on("end", () => resolve({ status: res.statusCode })); });
