...
import { CORPUS_ACCURACY_GATE, CORPUS_BENCHMARK_MIN_ITEMS, annotationAgreement, assertCorpusSchema, corpusCoverage, corpusScript, scoreCorpus, secondAnnotatorLabel, } from "./fixtures/messy-corpus.js";
...
test("schema validation rejects a corpus item missing provenance", () => {
    assert.throws(() => assertCorpusSchema([{ id: "x", text: "t", expectedClass: "news", source: "", license: "MIT", provenance: "real" }]), /missing source\/license/);
    assert.throws(() => assertCorpusSchema([
...
test("scoreCorpus scores only scorable items but checks structure on all", () => {
    ...
    const dropped = { ...perfect, classifications: perfect.classifications.slice(0, -1) };
    ...
    const badShape = perfect.classifications.map((entry, index) => index === hostileIndex ? { classification: "OK" } : entry);
