...
import { MAX_INLINE_SNAPSHOT_BYTES, deserializeWorkspaceSnapshot, serializeWorkspaceSnapshot, } from "../src/index.js";
function snapshot(overlay, links = {}) {
    return {
        id: "ws_test",
...
test("an overlay above the ceiling fails loudly instead of bloating history", () => {
    assert.throws(() => serializeWorkspaceSnapshot(snapshot({ "big.bin": new Uint8Array(MAX_INLINE_SNAPSHOT_BYTES + 1) })), (error) => {
