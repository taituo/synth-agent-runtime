...
import { ExecutionBroker, MemoryWorkspace, SyntheticExecutor, } from "../src/index.js";
import { FAULT_MATRIX, NOT_COVERED_FAULTS, REQUIRED_PROVEN_FAULT_IDS, REQUIRED_REASONED_FAULT_IDS, } from "./fixtures/fault-matrix.js";
function context(workspaceId) {
    return { agentId: "agt_matrix", workspaceId };
}
...
test("proven rows cite an EXECUTED live artifact; reasoned rows say why", () => {
    ...
    for (const id of NOT_COVERED_FAULTS)
        assert.equal(byId.has(id), false, `${id} must not be a matrix row`);
...
test("the executor rows' synthetic claim matches the synthetic rung's real behaviour", async () => {
    ...
    const result = await broker.execute({ id: "matrix-exec", kind: "process.exec", command: "echo hi" }, { agentId: "agt_matrix", workspaceId: workspace.id });
...
test("synthetic rung: workspace effects succeed, process.exec escalates", async () => {
    ...
    assert.equal(new TextDecoder().decode(read.output), "hi");
