...
test("a workspace.export effect puts the receipt's artifact reference in the store", async () => {
    ...
    try {
        ...
        const result = await broker.execute({ id: "e-export", kind: "workspace.export" }, { agentId: "agt_blob", workspaceId: workspace.id });
...
test("workspace.export without a blob store fails loudly rather than faking a reference", async () => {
    ...
    const result = await broker.execute({ id: "e-export-2", kind: "workspace.export" }, { agentId: "agt_blob", workspaceId: workspace.id });
