...
async function makeTaskRepo(parent, lib = BUGGY) {
...
async function patchFor(repo, mutate) {
    await mutate(repo);
...
test("a patch that exits 0 at import without fixing the bug does not pass", async () => {
    ...
    try {
        const repo = await makeTaskRepo(parent);
        const patch = await patchFor(repo, (dir) => writeFile(join(dir, "lib.mjs"), `process.exit(0);\nexport function slugify(text) {\n  return String(text).toUpperCase().replace(/[^A-Z0-9]+/g, "-");\n}\n`));
        const score = await scoreGymPatch({ patchText: patch, baseRepoDir: repo, cases: CASES });
...
test("a patch that monkeypatches assert does not pass", async () => {
    ...
    try {
        const repo = await makeTaskRepo(parent);
        const patch = await patchFor(repo, (dir) => writeFile(join(dir, "lib.mjs"), `import assert from "node:assert/strict";\nassert.equal = () => {};\nexport function slugify(text) {\n  return String(text).toUpperCase().replace(/[^A-Z0-9]+/g, "-");\n}\n`));
        const score = await scoreGymPatch({ patchText: patch, baseRepoDir: repo, cases: CASES });
...
test("a legacy hiddenTestPath call is errored, not a crash", async () => {
    ...
    try {
        ...
        const legacy = { patchText: patch, baseRepoDir: repo, hiddenTestPath: "hidden.test.mjs", expectedHiddenTests: 2 };
...
test("a case that outlives the timeout is errored, not passed", async () => {
    ...
    try {
        const repo = await makeTaskRepo(parent);
        const patch = await patchFor(repo, (dir) => writeFile(join(dir, "lib.mjs"), `export async function slugify() { await new Promise((r) => setTimeout(r, 5_000)); return ""; }\n`));
