...
export class CommandCoordinator {
    ...
    ownerId;
    ttlMs;
    constructor(state, leases, ownerId, ttlMs = 30_000) {
        this.state = state;
        this.leases = leases;
            ...
            run: async (lease, signal) => {
                ...
                if (existing?.status === "started") {
                    if (!options.reconcile)
                        throw new Error(`COMMAND_RECONCILIATION_REQUIRED:${options.id}`);
                    ...
                    if (resolution.status === "unknown")
                        throw new Error(`COMMAND_RECONCILIATION_REQUIRED:${options.id}:${resolution.detail ?? "unknown"}`);
                }
                const now = Date.now();
                const started = {
                    id: options.id,
                ...
                try {
                    ...
                    if (signal.aborted)
                        throw signal.reason ?? new Error(`LEASE_LOST:command:${options.id}`);
