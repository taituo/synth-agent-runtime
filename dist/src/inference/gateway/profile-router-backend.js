 ...
 */
export class ProfileRouterBackend {
    #backends;
    #profiles;
    #healthCache = new Map();
    ...
    async handle(request, model) {
        ...
        if (!profile)
            return jsonError(404, `Unknown routed model '${model}'`);
        let originalBody;
        try {
        ...
        for (const route of ordered) {
            ...
            if ((health.cooldownUntil ?? 0) <= this.now() && this.#backends.has(route.backend))
                candidates.push(route);
        }
        if (candidates.length === 0)
            return jsonError(503, `No healthy route for '${model}'`);
        let lastError;
        for (let index = 0; index < candidates.length; index++) {
            ...
            try {
                ...
                if (!retryableStatus(response.status)) {
                    ...
                    if (affinityKey && response.ok)
                        await this.#putAffinity(affinityKey, route.id);
                    return response;
                }
                health.failures++;
                health.cooldownUntil = this.now() + cooldownFor(response, route.cooldownMs, this.now());
                await this.#putHealth(model, route.id, health);
                if (affinityKey && stickyRoute === route.id)
                    await this.#deleteAffinity(affinityKey);
    ...
    async #putHealth(model, id, health) {
        const key = `${model}:${id}`;
...
function sessionAffinityKey(request, model, body) {
    ...
    if (headerSession)
        return `${tenant}:${model}:${headerSession}`;
    try {
        const parsed = JSON.parse(new TextDecoder().decode(body));
