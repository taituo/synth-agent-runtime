...
export function providerProfileId(provider) {
    return provider.profile ?? provider.id;
}
function requireString(value, field) {
    if (typeof value !== "string" || value.trim() === "")
        throw new Error(`provider config: ${field} must be a non-empty string`);
    return value;
}
/** Validate an untrusted config (env/JSON/file) into a `GatewayConfig`. */
export function parseGatewayConfig(input) {
    const raw = Array.isArray(input) ? { providers: input } : input;
    ...
    const providers = raw.providers.map((entry, index) => {
        if (!entry || typeof entry !== "object")
            throw new Error(`provider config: providers[${index}] must be an object`);
        const p = entry;
        const provider = {
        ...
        if (p.apiKey !== undefined)
            provider.apiKey = requireString(p.apiKey, `providers[${index}].apiKey`);
        if (p.profile !== undefined)
            provider.profile = requireString(p.profile, `providers[${index}].profile`);
        if (p.headers !== undefined) {
            if (typeof p.headers !== "object" || p.headers === null)
                throw new Error(`provider config: providers[${index}].headers must be an object`);
            provider.headers = Object.fromEntries(Object.entries(p.headers).map(([k, v]) => [k, String(v)]));
        }
        if (p.cooldownMs !== undefined) {
            if (typeof p.cooldownMs !== "number" || !Number.isFinite(p.cooldownMs))
                throw new Error(`provider config: providers[${index}].cooldownMs must be a number`);
    ...
    for (const provider of providers) {
        if (seen.has(provider.id))
            throw new Error(`provider config: duplicate provider id '${provider.id}'`);
...
export function buildProviderRouter(config, options = {}) {
    const backends = new Map();
    const profiles = new Map();
    for (const provider of config.providers) {
        const profileId = providerProfileId(provider);
        const backendName = `provider:${provider.id}`;
        const upstream = { id: provider.model, object: "model", provider: provider.id, profile: profileId };
...
export function selectProvider(config, idOrProfile) {
 ...
 */
export function directProviderSettings(provider) {
