import { normalizeReportedFinding } from "./findings.js";
import { streamToJsonl } from "./stream.js";
export const EVENTS_FILE = "events.jsonl";
export const FINDINGS_FILE = "findings.jsonl";
export const SWARM_TOOL_DEFINITIONS = [
...
export async function materializeStream(runner, stream, eventsPath = EVENTS_FILE) {
    try {
...
export function createSwarmTools(runner, options) {
    ...
    return {
        ...
        async execute(call) {
            ...
            if (call.name === "report_finding") {
                ...
                if (!finding)
                    return { ok: false, observation: "invalid finding: kind must be incident|slow-burn|correlation" };
